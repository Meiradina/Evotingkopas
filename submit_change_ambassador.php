...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = "INSERT INTO voting_ambasador (nama_pemilih, question_1, question_2, question_3, question_4, question_5, question_6, question_7, question_8, question_9, question_10) 
              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    if ($stmt = $conn->prepare($query)) {
        ...
        if ($stmt->execute()) {
            ...
            echo "<script>
                    alert('Data voting berhasil terkirim.');
                    window.location.href = 'katagory.php'; // Ganti dengan halaman yang diinginkan
                  </script>";
            exit();
        } else {
            echo "Terjadi kesalahan saat menyimpan voting: " . $stmt->error;
        }
